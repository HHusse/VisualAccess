...
import json
import cv2
import silent_face_anti_spoofing.test as anti_spoofing
...
def verify_face(image, known_faces, known_ids):
    ...
    for unknown_face_encoding in unknown_face_encodings:
        ...
        if True in matches:
            ...
            result = anti_spoofing.test(image, "./silent_face_anti_spoofing/resources/anti_spoof_models", 0)
            if result != True:
                logging.warning(f"Fake face detected for face id: {known_ids[first_match_index]}")
                return -1
            else:
