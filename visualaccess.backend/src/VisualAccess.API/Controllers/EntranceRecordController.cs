...
public class EntranceRecordController : ControllerBase
{
    ...
    public async Task<IActionResult> GetEntranceRecordsByPage([FromRoute] int page)
    {
        ...
        (ServiceResult, List<EntranceRecord>?) result = await getEntranceRecordsByPageService.Execute(page);
        return result.Item1 switch
        {
            ServiceResult.OK => StatusCode(200, result.Item2!.Select(record => new
            {
                record.username,
                record.roomName,
                record.time,
